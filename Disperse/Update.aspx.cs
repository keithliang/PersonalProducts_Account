    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            try   //==== 以下程式，只放「執行期間」的指令！=====================
            {
                ...
                dr_Month = cmd_Month.ExecuteReader();//---- 這時候執行SQL指令，取出資料
    ...
    {
        {
            ...
            try  //==== 以下程式，只放「執行期間」的指令！=====================
            {
                ...
                string MyTitle;
                myTable.Rows.Add(BuildNewRow("Order_date", "Assign_numbers", "Account_name", "Account_numbers", "Account_abstract", "Income", "Spend", "Menu"));
                while (dr.Read())
                {
                    MyTitle = "<A href=UpdateEdit.aspx?id=" + dr["id"] + ">edit</A>";
                    myTable.Rows.Add(BuildNewRow(Convert.ToDateTime(dr["Order_date"]).ToShortDateString(), dr["Assign_numbers"].ToString(), dr["Account_numbers"].ToString(), dr["Account_name"].ToString(), dr["Account_abstract"].ToString(), dr["Income"].ToString(), dr["Spend"].ToString(), MyTitle));
...
try  //==== 以下程式，只放「執行期間」的指令！=====================
    ...
    HtmlTableRow BuildNewRow(string Cell1Text, string Cell2Text, string Cell3Text, string Cell4Text, string Cell5Text, string Cell6Text, string Cell7Text, string Cell8Text)
    {
        ...
        cell8.Controls.Add(new LiteralControl(Cell8Text));
        cell8.Width = "20px";
        cell8.Style.Add("border-bottom", "1px dotted black");
        row.Cells.Add(cell8);
